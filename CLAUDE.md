...
- `GSView/GaussianPointRenderer.{h,cpp}`（`GSView::GaussianPointRenderer`）— `RenderMode::GaussianPoint` の GPU compute レンダラ（Phase 2 MVP）。毎フレーム GPU 上で clear → `gps_splat.comp`(depth pass / color pass) → `gps_resolve.comp` → `gps_composite`（swapchain）。**32-bit two-pass**（`shaderBufferInt64Atomics` 非依存）。compute は `GSViewApp::onPreRender` で frame command buffer に記録、composite は `GSViewRenderer::onRender`。`GSViewRenderer` が所有。DC 色・単色背景・caller 駆動カメラの MVP（SH・時間蓄積・occlusion culling・scan/compaction なし）。
